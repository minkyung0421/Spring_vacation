...
public class SelectTest2 {
	...
	public List<Singer> query(int member){
		String query = 
				"select * from singer_group where member=?";
	...
	public Singer queryForObject(String name){
		...
		= jdbcTemplate.queryForObject(query, 
				//테이블의 컬럼 네임이랑 멤버변수 이름이랑 같으면 자동으로 맵핑
				new BeanPropertyRowMapper<Singer>(Singer.class),
				name);
	...
	public void update(String name){
		String query
		= "update singer_group set member=6 where name=?";
	...
	public void update(Singer singer){
		String query
		= "update singer_group set member=? where name=?";
		
		int updateCount
		= jdbcTemplate.update(query, singer.getMember(), singer.getName());
	...
	public void delete(String name){
		String query
		= "delete from singer_group where name=?";
	...
	public static void main(String[] args) {
		...
		= ctx.getBean("selectTest2", SelectTest2.class);
